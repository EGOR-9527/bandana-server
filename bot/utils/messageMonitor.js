...
async function sendDocumentToOwner(filePath, caption = "") {
  try {
    ...
    formData.append("document", await fs.readFile(filePath), path.basename(filePath));
...
async function exportAllData() {
  try {
    ...
    await sendToOwner("📊 <b>Начинаю экспорт данных...</b>\nЭто может занять некоторое время.");
    ...
    chats.forEach((c, i) => {
      ...
      if (c.members_count) chatsContent += `    Участников: ${c.members_count}\n`;
      if (c.admins_count) chatsContent += `    Администраторов: ${c.admins_count}\n`;
      if (c.description) chatsContent += `    Описание: ${c.description.substring(0, 200)}${c.description.length > 200 ? "..." : ""}\n`;
    ...
    while (offset < totalMessages) {
      ...
      messages.forEach((m, i) => {
        ...
        if (m.file_id) {
          ...
          if (m.file_size) fileContent += `    Размер: ${(m.file_size / 1024).toFixed(2)} KB\n`;
        }
      ...
      const messageFile = path.join(messagesDir, `messages_part_${fileIndex}.txt`);
    ...
    await sendDocumentToOwner(zipPath, `📊 Экспорт данных бота\nДата: ${new Date().toLocaleString("ru-RU")}\nПользователей: ${users.length}\nЧатов: ${chats.length}\nСообщений: ${totalMessages}`);
